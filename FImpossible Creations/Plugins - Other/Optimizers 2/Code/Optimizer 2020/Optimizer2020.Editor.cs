    ...
    public partial class Optimizer2
    {
        ...
        public override void AssignComponentsToOptimizeFrom(Component target, bool includeAdvanced = false)
        {
            ...
            for (int i = 0; i < allOnTarget.Length; i++)
            {
                ...
                var instance = LODInstanceGenerator.GenerateInstanceOutOf(allOnTarget[i], true, includeAdvanced ? LODInstanceGenerator.ESearchMode.AllComponents : LODInstanceGenerator.ESearchMode.JustUnityComponents);
        ...
        public override void AssignCustomComponentToOptimize(MonoBehaviour target)
        {
            ...
            for (int i = 0; i < allOnTarget.Length; i++)
            {
                var instance = LODInstanceGenerator.GenerateInstanceOutOf(allOnTarget[i], true, LODInstanceGenerator.ESearchMode.JustCustomComponents);
                if (instance == null) continue;
