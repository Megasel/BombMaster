...
namespace FIMSpace.FOptimizing
{
    public partial class Optimizer_BaseEditor : Editor
    {
        ...
        public class EssentialOptimizerEditor : Optimizer_BaseEditor
        {
            ...
            [MenuItem("CONTEXT/EssentialOptimizer/Switch displaying Optimizer 2019.4+ Info + Converter")]
            ...
            public override void OnInspectorGUI()
            {
                ...
                if (EditorPrefs.GetInt("FOpt2019", 1) == 1)
                {
                    ...
                    EditorGUILayout.HelpBox("With Unity Version 2019.4+ consider using 'Optimizer (2019.4+)' component. (More stable solution for creating custom components LOD settings)", MessageType.Info);

                    if (GUILayout.Button(new GUIContent(" Convert ", "Replace 'Essential Optimizer' with 'Optimizer 2019.4+' component"), GUILayout.Height(36)))
                    {

                        Optimizer2 nOpt = Get.gameObject.AddComponent<Optimizer2>();
                        ...
                        {
                            try { field.SetValue(nOpt, field.GetValue(Get)); } catch (System.Exception) { }
                        }

                        // Mimicing LOD Settings
                        nOpt.ToOptimize.Clear();
                        nOpt.LODLevels = DGet.LODLevels;

                        for (int i = 0; i < DGet.ToOptimize.Count; i++)
                        {
                            nOpt.AddToOptimizeIfCan(DGet.ToOptimize[i].Component, null);
                        }
                        ...
                        {
                            if (nOpt.ToOptimize[i].Component != DGet.ToOptimize[i].Component) continue;
                            nOpt.ToOptimize[i].SetFromEssential(DGet.ToOptimize[i]);
                        }
